...
<div class="container-fluid py-4">
        ...
        <div class="col-lg-12 mb-md-0 mb-4">
            <div class="card">
                ...
                <button class="open-btn" onclick="openModal()">Add New <?= $nameBtn; ?></button>
                    <button class="open-btn" id="btnDeleteSelected" style="background-color:#d33;color:#fff;display:none;margin-left:10px;" onclick="bulkDeleteBrand()">Delete Selected (<span id="selectedCount">0</span>)</button>
                            ...
                            <tbody>
                                <?php if ($result->num_rows > 0): ?>
                                    <?php while ($row = $result->fetch_assoc()): ?>
                                        <?php $countC = getUsedBrand($row['id']); ?>
                                        <tr>
                                            <td>
    ...
    margin-bottom: 10px;color:blue;cursor:pointer;"
                                                    onClick="window.location.href = '<?= $domainURL; ?>update-brand/<?= $row['id'] ?>'"></i>
        ...
        margin-bottom: 10px;color:grey;cursor:no-drop;"
                                                        onclick="alert('Cannot delete \'Brand: <?= $row['name']; ?>\'. <?= $countC['count']; ?> product was attached to this brand.')"></i>
                                                    <?php
                                                }
                                                ?>
                                            ...
                                            <!-- <img src="<?= $domainURL ?>assets/images/brand-category/<?= htmlspecialchars($row['image']) ?>" width="50"> -->
                                            </td>
                                            <td>
                        ...
                        <script>
                            ...
                            function bulkDeleteBrand() {
                                ...
                                Swal.fire({
                                    title: 'Delete ' + ids.length + ' brands?',
                                    ...
                                    if (result.isConfirmed) {
                                        $.ajax({
                                            url: '<?= $domainURL ?>bulk-delete-brand',
                                            method: 'POST',
                                            data: { ids: ids },
                                            ...
                                            error: function() {
                                                Swal.fire('Error', 'Failed to delete brands.', 'error');
                                            }
                                        });
                                    }
                                });
