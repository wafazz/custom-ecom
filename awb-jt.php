...
// Remove duplicates and keep as comma-separated
$printIDs = implode(',', array_unique(explode(',', $printID)));

$sqlPrint = "SELECT * FROM dhl_bulk_print WHERE id='$printIDs'";
$queryPrint = $conn->query($sqlPrint);
$rowPrint = $queryPrint->fetch_array();
// $order = getOrder(1, $orderID);

// $orerIDS = explode(",", $rowPrint["order_id"]);
$orerIDS = explode(",", $printIDs);

$idArray = explode(',', $printIDs);

$formattedIds = array_map(function($ide) {
    return '[' . trim($ide) . ']';
}, $idArray);

$finalString = implode(',', $formattedIds);

$addPrinted = $conn->query("INSERT INTO `awb_printed`(`id`, `order_id`, `created_at`, `updated_at`, `deleted_at`, `printed_by`) VALUES (NULL,'$finalString','$dateNow','$dateNow',NULL,'".$_SESSION['user']->id."')");
...
foreach ($orerIDS as $orerID) {
    ...
    $upPrinted = $conn->query("UPDATE customer_orders SET `status`='3', printed_awb='1' WHERE id='$orerID'");
    $getOrderss = "
    SELECT 
...
";
    ...
    while ($orders = $gr->fetch_array()) {
        ...
        $itemsddd .= '<tr style="border-bottom: 1px solid #ccc;">
            <td style="width:70%; text-align:left;padding: 5px 8px;font-size:15px;font-weight:bold;">' . $orders["product_name"] . '</td>
            <td style="width:30%; text-align:center;padding: 5px 8px;font-size:15px;font-weight:bold;">x' . $orders["quantity"] . '</td>
        </tr>';
